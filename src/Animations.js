...
/* Pilier de lumière du Paladin : colonne verticale qui jaillit sur l'ennemi
   ...
   Double colonne (voile large + cœur brillant) qui tournoie en s'éteignant. */
export function lightPillar(x, y, z, color) {
  ...
  const core = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.2, 4.6, 8, 1, true),
    new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.85,
      blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide }));
  core.position.set(x, y + 2.0, z);
  S.scene.add(core);
  fx.push({ m: core, t: 0, life: 0.3, kind: 'pillar' });
}
...
export function updateFx(dt) {
  for (let i = fx.length - 1; i >= 0; i--) {
    ...
    if (f.kind === 'arc') {
      ...
      f.m.rotation.y += dt * 2.6; // la colonne tournoie en s'éteignant
