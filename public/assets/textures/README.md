...
## Textures procédurales générées (végétation)

Trois familles supplémentaires, générées procéduralement (bruit de valeur
multi-octaves, tuilables par construction), calibrées pour l'éclairage
nocturne du jeu (leur teinte moyenne reprend celle des anciennes couleurs
unies de repli) :

| Famille | Fichiers | Utilisée par |
|---------|----------|--------------|
| `grass` | `grass_color.jpg` + `grass_normal.png` + `grass_roughness.jpg` | tout le sol extérieur (`grass`) |
| `hedge` | `hedge_color.jpg` + `hedge_normal.png` + `hedge_roughness.jpg` | haies des labyrinthes (`hedge`, `hedgeF`) |
| `leaf`  | `leaf_color.jpg` + `leaf_normal.png` + `leaf_roughness.jpg` | canopées primitives et touffes d'herbe (`leaf`) |
